...
def attach_branch(module: torch.nn.Module, l1: torch.Tensor, l2: torch.Tensor,
    ...
    On a GPU too small to hold the whole model, ComfyUI streams weights and the base
    module can run on CUDA while these buffers are still on CPU -- so the branch follows
    the activation's device at call time, caching the move so it costs nothing after the
    first step.
    """
    if not hasattr(module, "_branch_specs"):
        ...
        def forward(x, *args, **kwargs):
            ...
            for idx, mult, _ in module._branch_specs:
                n1 = "_br_l1_{}".format(idx)
                n2 = "_br_l2_{}".format(idx)
                a1 = getattr(module, n1)
                a2 = getattr(module, n2)
                if a1.device != x.device:
                    a1 = a1.to(device=x.device)
                    a2 = a2.to(device=x.device)
                    setattr(module, n1, a1)
                    setattr(module, n2, a2)
                a1 = a1.to(dtype=x.dtype)
                a2 = a2.to(dtype=x.dtype)
